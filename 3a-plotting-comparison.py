...
# for test in contrasts:
    ...
    # if op.exists(conn_csv):
        ...
        # sns.stripplot(
            ...
            # orient="h",
            # jitter=False,
            # palette={'Atlas ROI': '#792CA2', 'Drawn ROI': '#E05454'},
            # linewidth=0.8,
            # edgecolor="w",
            # alpha=0.85,
            # ax=ax
...
for test in contrasts:
    ...
    if op.exists(drawn_path) and op.exists(atlas_path):
        ...
        # Seaborn turns off the grid on the categorical (y) axis by default for
        # stripplot; re-enable it so both axes get the same whitegrid box grid.
        ax.yaxis.grid(True)
        sns.despine(ax=ax)
...
# for test in contrasts:
    ...
    # if op.exists(drawn_path) and op.exists(atlas_path):
        ...
        # sns.stripplot(
            ...
            # orient="h", 
            # jitter=False, 
            # palette={'Drawn ROI': '#E05454', 'Atlas ROI': '#792CA2'}, # Teal vs Dark Orange
            # linewidth=0.8, 
            # edgecolor="w", 
            # alpha=0.85,
            # ax=ax
