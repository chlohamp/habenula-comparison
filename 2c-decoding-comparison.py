...
for test_key, paths in contrasts_to_compare.items():
    ...
    # ===== SEMANTIC TERM CORRELATION SCATTERPLOT (Figure 4) =====
    # Seaborn scatterplot of Atlas vs. Subject-Specific term correlations across shared
    # NeuroQuery terms, annotated with the Pearson r / Spearman rho above.
    fig, ax = plt.subplots(figsize=(6, 6))
    sns.regplot(
        data=merged_decoding, x="Atlas_r", y="Drawn_r",
        scatter_kws={"s": 32, "alpha": 0.75, "color": "#792CA2"},
        line_kws={"color": "#E05454", "linewidth": 1.5}, ci=None, ax=ax,
    )
    ax.set_xlabel("Atlas ROI Term Correlation (r)")
    ax.set_ylabel("Subject-Specific ROI Term Correlation (r)")
    ax.set_title(
        f"Meta-Analytic Semantic Term Correlations ({test_key.upper()})\n"
        f"({len(merged_decoding)} shared NeuroQuery terms)"
    )
    ax.text(
        0.05, 0.95,
        f"Pearson r = {profile_pearson:.3f}\nSpearman $\\rho$ = {profile_spearman:.3f}",
        transform=ax.transAxes, fontsize=10, va="top",
    )
    sns.despine(ax=ax)
    fig.tight_layout()
    scatter_path = op.join(analysis_dir, f"term_correlation_scatter_{test_key}.png")
